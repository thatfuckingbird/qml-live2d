...
LAppModel::LAppModel(Live2DItemRenderer* renderer)
    : CubismUserModel()
    , renderer(renderer)
    , _modelSetting(NULL)
...
csmBool LAppModel::LoadAssets(const csmChar* dir, const csmChar* fileName)
{
    ...
    if (!SetupModel(setting)) return false;
...
{
    const csmFloat32 deltaTimeSeconds = renderer->getTimerData().getDeltaTime();
...
void LAppModel::SetupTextures()
{
    for (csmInt32 modelTextureNumber = 0; modelTextureNumber < _modelSetting->GetTextureCount(); modelTextureNumber++)
    {
        ...
        LAppTextureManager::TextureInfo* texture = renderer->getTextureManager()->CreateTextureFromPngFile(texturePath.GetRawString());
